...
public class Collision {
    ...
    public void checkTileCollision( Entity entity ) {
        entity.setCollisionUp(false); entity.setCollisionDown(false);
        entity.setCollisionLeft(false); entity.setCollisionRight(false);
            ...
            for (int tileX = startX; tileX <= endX; tileX++) {
                ...
                if (tileNum >= 0 && tileNum < gamePanel.map.getTile().length) { // check if tile number is valid
                    ...
                    if (tile != null && tile.getTileCollision()) { // if tile has collision will loop through all the collision areas
                        for (Rectangle rect : tile.getCollisionArea()) { // create then a new rectangle representing tile collision area
                            Rectangle tileRect = new Rectangle(tileX * tileSize + rect.x, tileY * tileSize + rect.y, rect.width, rect.height);

                            // check collision if player moves up INTERSECTS with tile collision area
                            Rectangle upRect = new Rectangle(playerRect.x, playerRect.y - entity.getSpeed(), playerRect.width, entity.getSpeed());
                            if (upRect.intersects(tileRect)) entity.setCollisionUp(true);

                            // check collision if player moves down INTERSECTS with tile collision area
                            Rectangle downRect = new Rectangle(playerRect.x, playerRect.y + playerRect.height, playerRect.width, entity.getSpeed());
                            if (downRect.intersects(tileRect)) entity.setCollisionDown(true);

                            // check collision if player moves left INTERSECTS with tile collision area
                            Rectangle leftRect = new Rectangle(playerRect.x - entity.getSpeed(), playerRect.y, entity.getSpeed(), playerRect.height);
                            if (leftRect.intersects(tileRect)) entity.setCollisionLeft(true);

                            // check collision if player moves right INTERSECTS with tile collision area
                            Rectangle rightRect = new Rectangle(playerRect.x + playerRect.width, playerRect.y, entity.getSpeed(), playerRect.height);
